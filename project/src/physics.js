class Mover {
  ...
  update(state) {
    this.checkBounds();
    // position
    vec3.scale(this.acceleration, this.acceleration, state.deltaTime);
    vec3.add(this.velocity, this.velocity, this.acceleration);
    var timeVelocity = vec3.clone(this.velocity); // Create duplicate to prevent change to original
    vec3.scale(timeVelocity, timeVelocity, state.deltaTime);
    vec3.add(this.position, this.position, timeVelocity);
    vec3.scale(this.acceleration, this.acceleration, 0);

    // rotation
    vec3.scale(this.rotation.accel, this.rotation.accel, state.deltaTime);
    vec3.add(this.rotation.vel, this.rotation.vel, this.rotation.accel);
    let timeRotateVel = vec3.clone(this.rotation.vel);
    vec3.scale(timeRotateVel, timeRotateVel, state.deltaTime);
    mat4.rotate(this.rotation.mat, this.rotation.mat, timeRotateVel[0], [1,0,0]); //x
    mat4.rotate(this.rotation.mat, this.rotation.mat, timeRotateVel[1], [0,1,0]); //y
    mat4.rotate(this.rotation.mat, this.rotation.mat, timeRotateVel[2], [0,0,1]); //z
